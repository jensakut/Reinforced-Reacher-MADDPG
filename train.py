...
def ddpg_unity(env, agent, brain_name, num_agents, plotting, n_episodes=200):
    """Train DDPG Agent
    Params
    ======
    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        for timestep in count():
            ...
            if np.any(dones):  # exit loop if episode finished
                plotting.add_measurement(score=np.mean(score), episode_length=timestep, epsilon=agent.epsilon)
                break
        ctime = time.time() - t_s
        ...
        print('\rEpisode {}\tAverage Score: {:.2f}\tScore: {:.2f}, Min Score: {:.2f}, Max Score: {:.2f}, '
            ...
            i_episode, np.mean(scores_deque), np.mean(score), np.min(score), np.max(score), ctime), end="")
        if i_episode % 10 == 0:
            torch.save(agent.actor_local.state_dict(), 'checkpoint_actor.pth')
            torch.save(agent.critic_local.state_dict(), 'checkpoint_critic.pth')
            plotting.plotting('ddpg_reacher', i_episode)
            print('\rEpisode {}\tAverage Score: {:.2f}\tScore: {:.2f}, Min Score: {:.2f}, Max Score: {:.2f}, '
                  'episode_dur {:.2f}'.format(
...
def train_in_unity_env():
    ...
    ddpg_unity(env, agent, brain_name, num_agents, plotting, n_episodes=200)
    env.close()


env = UnityEnvironment(file_name='Reacher_Linux_20_Agents/Reacher.x86_64')
# env = UnityEnvironment(file_name='Crawler_Linux_12_Agents/Crawler.x86_64')
